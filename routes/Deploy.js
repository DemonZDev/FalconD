...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    const container = const serverProcess = spawn('java', ['-Xmx4G', '-Xms2G', '-jar', 'server.jar', 'nogui'], {
    cwd: '/home/servers/minecraft',
    env: process.env,
    detached: true
});
...
router.delete("/:id", async (req, res) => {
  const container = // Track server process using PID instead of container ID;
  try {
...
router.post("/redeploy/:id", async (req, res) => {
  ...
  try {
    ...
    const newContainer = const serverProcess = spawn('java', ['-Xmx4G', '-Xms2G', '-jar', 'server.jar', 'nogui'], {
    cwd: '/home/servers/minecraft',
    env: process.env,
    detached: true
});
